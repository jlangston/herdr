...
/// Default mouse-active lone-escape flush timeout, in milliseconds. Derived
/// from the built-in `raw_input::MOUSE_ACTIVE_LONE_ESCAPE_FLUSH_TIMEOUT_MS`, so
/// leaving `[ui] escape_time_ms` unset preserves the built-in behavior exactly
/// and the two stay in sync.
pub const DEFAULT_ESCAPE_TIME_MS: u64 =
    crate::raw_input::MOUSE_ACTIVE_LONE_ESCAPE_FLUSH_TIMEOUT_MS as u64;
